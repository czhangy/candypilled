...
lets a `.sav`/`.dsv` file be parsed without leaving the modal — merged into an
...
- **On successful import (Import tab)** — `ImportSaveForm` calls
  `onImport` with the parsed data and protagonist gender, and requests
  the modal's animated close
