...
organized into tabs. Currently only shows a "Reset" tab (when a run
already exists for the game), which shows a description of what the
action does and a red confirm button, so the action can be triggered
...
| ----------- | --------- | ------------- | ------------------------------- |
| `activeTab` | `'reset'` | `'reset'`     | The currently selected data tab |
