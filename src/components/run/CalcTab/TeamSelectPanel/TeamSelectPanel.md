...
| ---------------------------- | ------------------------- | -------- | ------- | ---------------------------------------------------------------------------- |
| `disableSwitchInPredictions` | `boolean`                 | Yes      | -       | The "Disable Switch-In Predictions" setting; suppresses the order badges     |
...
- `switchInRanks` — team index -> 1-based predicted switch-in order for
  ...
  `target` isn't set, or `disableSwitchInPredictions` is set
