...
displays/edits a Pokémon's battle-relevant details: species name (suffixed
with a blue ♂ or pink ♀ gender symbol when `gender` is set) and level in a
combined row, nature and status condition in a row below that, ability and
held item in a row below that, stat spread (base/IV/EV/stage/total), and
moves. All editable values and their change
