...
- `baseStats` / `totalStats` — base stats resolved via `PokemonHelpers` for
  ...
  base stats and `mon`'s fixed level/IVs/EVs/nature, with each non-HP stat
  further adjusted by its `boosts` stage via `StatHelpers.applyBoost`, and
  Speed additionally doubled when `isTailwind` is true
