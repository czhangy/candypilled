...
whichever split is being viewed. Once the run has defeated every required
battle, the game is treated as complete and no level cap is shown.
...
| `defeatedBattles`  | `string[]`               | Yes      | -       | The run's defeated battle keys, used to detect game completion |
...
- `levelCap` — the highest level Pokémon on the team of the run's current
  ...
  the split title; `null` once the run has defeated every required battle,
