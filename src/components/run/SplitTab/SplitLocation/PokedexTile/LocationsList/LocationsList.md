...
A scrollable list of every location a Pokémon can be found in the wild,
...
no icon). When interactive, the entire row is a link (ignoring any
subarea qualifier in the location's name, since a split's location card
exists per location, not per subarea) to that location's card in the
Splits tab; otherwise rows are static, unclickable text. A row is
highlighted with a red background if its location's encounter has already
been used, whether by catching a Pokémon there or by marking it missed.
...
| ------------------ | ---------------------------- | -------- | ------- | ----------------------------------------------------------------------------------------------------------------- |
| `interactive`      | `boolean`                    | Yes      | -       | Whether rows render as clickable links or static text                                                             |
| `locations`        | `EncounterLocation[]`        | Yes      | -       | The locations to list, unsorted                                                                                   |
| `onSelectLocation` | `(location: string) => void` | Yes      | -       | Called with a location's base name (subarea stripped) when its row is clicked (only reachable when `interactive`) |
