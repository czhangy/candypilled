...
A form for recording the details of a Pokemon: its nature and IVs always,
plus species, level, ability, moves, and EVs when their respective
...
The ability dropdown identifies an ability by its slot (1, 2, or hidden)
rather than its name, since a species can be re-selected across
generations where ability names at a given slot may differ. When
`lockSpecies` is set, the Pokemon field isn't rendered at all and
`species` stays fixed at `defaultSpecies`. Each `default*` prop seeds its
corresponding field's initial state, falling back to a blank/default
value when omitted; none of the 4 move slots default to a value unless
`defaultMoves` is passed. Submitting the form passes the selected values
up. Used by `AddPokemonModal` (all fields but EVs shown, species
editable) when catching a Pokemon, by `StarterSelectModal` (level,
ability, moves, and EVs hidden, defaulting to level 5 and the species'
first ability slot; species field omitted, fixed to the already-chosen
starter) when configuring a chosen starter, and by `EditPokemonModal`
(all fields including EVs shown, species locked, every field seeded from
...
| `onSubmit`           | `(details: Pick<BattlePokemon, 'ability' \| 'evs' \| 'ivs' \| 'level' \| 'moves' \| 'name' \| 'nature'>) => void` | Yes      | -       | Called with the selected details when the form is submitted                          |
