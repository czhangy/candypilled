...
`Dropdown` (species, ability, and moves are searchable). When
`lockSpecies` is set, the Pokemon field isn't rendered at all and
`species` stays fixed at `defaultSpecies`. None of the 4 move slots
default to a value. Submitting the form passes the selected values up.
Used by `AddPokemonModal` (all fields shown, species editable) when
catching a Pokemon and by `StarterSelectModal` (level, ability, and moves
hidden, defaulting to level 5 and the species' first ability; species
field omitted, fixed to the already-chosen starter) when configuring a
chosen starter.
