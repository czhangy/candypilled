...
A search bar above a scrollable list of named items. Clicking an item
selects it; the currently selected item is highlighted. Typing in the
search bar filters the list to items whose name contains the query
...
| `sortAlphabetically` | `boolean`                          | Yes      | -       | Whether matching items are re-sorted alphabetically by name, or shown in `items`' given order (e.g. dex number order) |
...
- `filteredItems` — `items` filtered by `query` (case-insensitive substring
  match against each item's name)
- `visibleItems` — `filteredItems`, sorted alphabetically by name when
  `sortAlphabetically`, otherwise left in `items`' given order
